...
def Calc_FlowVarResults( Gamma, Tot_X_Pts, A, InitVal):
    #CALC_FLOWVARRESULTS finds physical flow variables from simulation results
    #   Calc_FlowVarResults finds Mach number and dimensionless mass density,
    #       pressure, temperature, and velocity at end of subinterval i at 
    ...
    Mach_D = np.zeros((Tot_X_Pts))
    Mrho_D = np.zeros((Tot_X_Pts))
    Press_D = np.zeros((Tot_X_Pts))
    Temp_D = np.zeros((Tot_X_Pts))
    Vel_D = np.zeros((Tot_X_Pts))
    ...
    for i in range(Tot_X_Pts + 1):
        Mrho_D[i] = InitVal[0, i]/A[i]
        Vel_D[i] = InitVal[1, i]/InitVal[0, i]
        Temp_D[i] = fac1*( (InitVal[2, i]/InitVal[0, i])- fac2*Vel_D[i]*Vel_D[i] )
