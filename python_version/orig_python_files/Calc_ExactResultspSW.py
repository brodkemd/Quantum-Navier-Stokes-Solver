...
def Calc_ExactResultspSW(Gamma, Tot_X_Pts, A, Exit_Pressure, Del_x):
    ...
    #       Flow", (MCF) Chapters 3 and 5. See also: Anderson, "Computational 
    #       Fluid Dynamics", (CFD) Chapter 7 and entries for 07/30/19, 
    #       07/31/19, and 08/01/19 in my research notebooks.
    #
    #   INPUTS:
    ...
    ratioPA = Exit_Pressure*A[Tot_X_Pts - 1] # Eq.(7.126a) in CFD

    arg1 = fac2 + fac3*( fac4**(exponent1) )*(1/ratioPA)**(2)

    M[Tot_X_Pts - 1] = np.sqrt( fac1 + np.sqrt(arg1) ) # Eq.(5.28) in MCF

    arg2 = 1 + fac5*( M[Tot_X_Pts - 1] )**(2)
    ...
    M[ithroat - 1] = 1        # Eq.(5.20) in MCF

    # evaluate other flow variables at throat

    argum = 1 + fac5*(M[ithroat- 1])**(2)

    Press[ithroat - 1] = (argum)**(-1 * exponent2)     # Eq.(3.30) in MCF
    Mrho[ithroat - 1] = (argum)**(fac1)            # Eq.(3.31) in MCF
    Temp[ithroat - 1] = 1/argum                   # Eq.(3.28) in MCF

    Vel[ithroat - 1] = M[ithroat - 1]*np.sqrt(Temp[ithroat - 1]) # Eq. (7.76) in CFD
    ...
    SW_JumpM = np.sqrt( (1 + fac5*M1**2)/(Gamma*M1**2 - fac5) ) # Eq.(7.138) CFD
    ...
    arg7 = fac4*( 1 + fac5*( M[Tot_X_Pts - 1] )**(2) )

    # derivation of following formula in my research notebook, 07-31-19 entry,
    #   page 11

    AstarE = A[Tot_X_Pts - 1]*M[Tot_X_Pts - 1]*( arg7 )**(-1 * exponent3)

    # begin loop over downstream grid-points

    for i in range(int(iswp1)-1, Tot_X_Pts):
    ...
    for i in range(int(iswp1) -1, Tot_X_Pts):
        ...
        Press[i] = ratioP21*(arg6)**(-1 * exponent2)
    ...
    # finally, rescale Mrho, Press, and Temp by their values at nozzle 
