from Set_XGrid import Set_XGrid
from Calc_Noz_Area import Calc_Noz_Area
from Calc_ExactResultsmSW  import Calc_ExactResultsmSW
from Calc_ExactResultspSW import Calc_ExactResultspSW
from SetInCond import SetInCond
from InitParms import InitParms
from IPrtn import IPrtn
import numpy as np


def InitCalcParms(x_min, x_max, Tot_X_Pts, Gamma, Exit_Pressure, Shock_Flag, In_Mass_Flow, d, ICMFlowErrScale, ICrhoErrScale, ICtempErrScale, in_n, delta, err1, Tot_TSteps, a):
    ...
    #               in sub-subinterval j is t(j-1,i) and the initial time a is
    #               NOT stored in t.
    ...
    #   computational flow variables U(d,Tot_X_Pts)
    '''
    print("Shock_Flag =", Shock_Flag)
    print("In_Mass_Flow=", In_Mass_Flow)
    print("Gamma=", Gamma)
    print("x=", x)
    print("Del_x=", Del_x)
    print("A=", A)
    ...
    print("Mrho_E=", Mrho_E)
    print("Temp_E=", Temp_E)
    print("ICMFlowErrScale=", ICMFlowErrScale)
    print("ICrhoErrScale=", ICrhoErrScale)
    print("ICtempErrScale=", ICtempErrScale)
    '''
    ...
    U2 = np.zeros((int(Tot_X_Pts), int(n+1)))

    # use initial condition to assign U2 at start of subinterval i = 1

    for gridpt in range(int(Tot_X_Pts)):
        U2[gridpt, 0] = InitVal[1, gridpt]
    ...
    ff0_throat = np.zeros((int(d),int(n)))
    ff1_throat = np.zeros((int(d),int(n)))
    ff2_throat = np.zeros((int(d),int(n)))

    # output initial condition for runtime inspection

    #usr_message = 'Calculation paused - press any key to continue: '

    #print("InitVal =", np.transpose(InitVal))
    ...
    #print("n =", n)
    #print("k =", k)
    #print("In_Mass_Flow_Noisy =", In_Mass_Flow_Noisy)

    #if Shock_Flag == 1:
    #    SW_JumpP = float(SW_JumpP)
    #    SW_JumpM = float(SW_JumpM)
    #    print("SW_Jump =", SW_JumpP)
    #    print("SW_Jump =", SW_JumpM)
