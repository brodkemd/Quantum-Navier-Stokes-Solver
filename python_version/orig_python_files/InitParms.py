...
def InitParms( n, delta, err1, Tot_TSteps ):
    ...
    k = 1+ np.ceil(np.log(1/err1)/np.log(n))
    ...
    final_n = np.ceil(temp_n)

    k = 1 + np.ceil(np.log(1/err1)/np.log(final_n))
