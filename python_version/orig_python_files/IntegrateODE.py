from BldTPoly import BldTPoly
from IntegrateGij import IntegrateGij
from CalcBCmSW import CalcBCmSW
from CalcBCpSW import CalcBCpSW
from Calc_FlowVarResults import Calc_FlowVarResults
import numpy as np

def IntegrateODE(d, n, N, hbar, r, Del_x, Gamma, Tot_Int_Pts, k, Tot_X_Pts, Shock_Flag, Exit_Pressure, ithroat, a, delta1, rho, InitVal, A, t, U2_in, ff0_throat_in, ff1_throat_in, ff2_throat_in, Mach_E, Mrho_E, Press_E, Temp_E, Vel_E, In_Mass_Flow):
    ...
    #                       number at each grStoreTimes4i[j]toring relative error in
    ...
    for i in range(int(n)):
        ...
        if i == n - 1:
            ...
            Rel_MachErr = np.divide(np.abs(Mach_D - Mach_E), Mach_E)
            Rel_MrhoErr = np.divide(np.abs(Mrho_D - Mrho_E), Mrho_E)
            Rel_PressErr = np.divide(np.abs(Press_D - Press_E), Press_E)
            Rel_TempErr = np.divide(np.abs(Temp_D - Temp_E), Temp_E)
            Rel_VelErr = np.divide(np.abs(Vel_D - Vel_E), Vel_E)

            # calculate mean mass flow rate at final time define array to store

            MeanU2 = np.mean(U2[:, n+1])

            AvU2 = np.zeros(Tot_X_Pts)
            ...
            AvRelTempErr = np.zeros(Tot_X_Pts)
            AvRelMachErr = np.zeros(Tot_X_Pts)
            AvRelMrhoErr = np.zeros(Tot_X_Pts)
            AvRelPressErr = np.zeros(Tot_X_Pts)

            AvPlusSDevRelTempErr = np.zeros(Tot_X_Pts)
            AvPlusSDevRelMachErr = np.zeros(Tot_X_Pts)
            AvPlusSDevRelMrhoErr = np.zeros(Tot_X_Pts)
            AvPlusSDevRelPressErr = np.zeros(Tot_X_Pts)

            AvMinusSDevRelTempErr = np.zeros(Tot_X_Pts)
            AvMinusSDevRelMachErr = np.zeros(Tot_X_Pts)
            AvMinusSDevRelMrhoErr = np.zeros(Tot_X_Pts)
            AvMinusSDevRelPressErr = np.zeros(Tot_X_Pts)
    ...
    return U2, Mach_D, Mrho_D, Press_D, Temp_D, Vel_D, Rel_MachErr, Rel_MrhoErr, Rel_PressErr, Rel_TempErr, Rel_VelErr, AvRelTempErr, AvPlusSDevRelTempErr, AvMinusSDevRelTempErr,AvRelMachErr, AvPlusSDevRelMachErr, AvMinusSDevRelMachErr,AvRelMrhoErr, AvPlusSDevRelMrhoErr, AvMinusSDevRelMrhoErr,AvRelPressErr, AvPlusSDevRelPressErr, AvMinusSDevRelPressErr, AvU2, ff0_throat, ff1_throat, ff2_throat